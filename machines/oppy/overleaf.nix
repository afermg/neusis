...
in
{
  ...
  # Match Moby's runtime exactly. In particular, the copied database still has
  # FCV 7.0 and cannot be opened directly by the MongoDB 8.2 in nixos-25.11.
    ...
    tables.overleaf-origin = {
      ...
      content = ''
        chain input {
          ...
          iifname "tailscale0" ip daddr ${privateOriginAddress} tcp dport 18080 drop
        }
      '';
    };
  };
  ...
  # Force the same Node, TeX, PDF, and shell tool closure used by Moby instead
  # of silently mixing these services with Oppy's older nixos-25.11 packages.
  systemd.services =
